...
Решением считается репозиторий на github.com c кодом в jupyter notebook (очень хорошо и подробно описанным), который должен без ошибок запускаться по кнопке Run All. Пример такого репозитория от организаторов хакатона можно найти [тут]().
