...
Решением считается репозиторий на [github](https://github.com) c кодом в jupyter notebook (очень хорошо и подробно описанным + всеми данными, необходимыми для его запуска), который должен без ошибок запускаться по кнопке Run All. Пример такого репозитория от организаторов хакатона можно найти [тут]().
